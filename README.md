# Mule Kick: SFDC to SFDC Opportunity Migration
...
This Kick (template) should serve as a foundation for the process of migrating Opportunities from one Salesfoce instance to another, being able to specify filtering criterias and desired behaviour when an Opportunity already exists in the destination org. 
...
During the Input stage the Kick will go to the SalesForce Org A and query all the existing Opportunities that match the filter criteria.
